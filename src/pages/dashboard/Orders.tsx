  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Track all your gadget orders</p>
      </div>
      <div className="rounded-xl border bg-card shadow-card overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b text-left text-muted-foreground">
            <th className="p-4 font-medium">Order ID</th><th className="p-4 font-medium">Product</th><th className="p-4 font-medium">Date</th>
          ...
          <tbody>
            {mockOrders.map(o => (
              <tr key={o.id} className="border-b last:border-0 hover:bg-muted/50">
                ...
                <td className="p-4 font-medium text-card-foreground">{formatCurrency(o.amount)}</td>
